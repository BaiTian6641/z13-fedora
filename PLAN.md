...
| Android (Track A, default) | Fedora `waydroid` 1.6.3 + **official channel: LineageOS 20 / Android 13 GAPPS** (`ota.waydro.id`), baked at `/usr/share/waydroid-extra/images`, rendering on the **Iris Xe** iGPU | Binder is in-kernel; iGPU is Waydroid's best-supported path. The ATV Android 16 channel crash-looped on-device (and is a TV form factor) - switched to stock; `--atv` remains as a test flag. |
...
| CI | `build.yml` (daily + push + PR, recipe matrix) gates on a `detect` job: with `SIGNING_SECRET` it runs `publish` (the official action, signed, rechunked to ≤128 layers); without it `validate` runs the CLI **directly** (`push: false`, chunked) with `tee` + an artifact upload, because action logs and artifacts both need an authenticated API call — on failure the log tail is committed to `.ci/last-failure.md`, which a plain `git pull` reads. Real content errors therefore surface before the key exists. `iso.yml` builds the offline installer (checksum, release attach) behind a `preflight` job that requires a successful `Build and publish the image` job in the triggering run |
...
| Layer-count limit | the ublue base carries **259 layers** and the build-container-installer the CLI pins (`v1.4.0`, Oct 2025) predates BCI PR #205 ("allow for images with more than 128 layers", merged 2026-07-13, whose body reads *"Ublue has over 128 layers, which prevents the image from being pulled with docker"*). **Chunking has been dropped entirely.** After the ISO job moved to BCI v1.5.0 (which handles >128 layers, PR #205), chunking had no remaining purpose - and the chunked path became the failure point: four publishes died silently during the staging push of the `..._unchunked` image (the log shows the blob list, then nothing - a kill, not an error), while the plain build path was proven green twice. The publish is now a plain build+push; the image carries ~259 layers, which the ISO builder pulls fine — measured, not assumed: at `max_layers: 128` the published image came out at **129 layers** (128 packed plus the base) and the ISO step failed to pull it, which is exactly the failure mode BCI PR #205 describes — chunkah packs components to fit the ceiling (`coreos/chunkah`: default 64, and containers-storage's hard limit is 500), and the published image measures **129 layers** (128 packed plus the base), which the ISO step pulls with podman/skopeo. The CI check now resolves the platform manifest before counting (a tag is an index, so the old check read 0 and passed vacuously) and warns above Docker's recommended 128 while failing only past 500 |
...
| `bluebuild` | `3d405c0` | failure (13 min) | same failure point; the capture failed again because `git commit -am` never stages a *new* `.ci/last-failure.md`. The publish job now runs the CLI directly (as `validate` already does, with a `tee`d log) instead of the opaque action step, and the capture does a fresh clone before committing |
...
| `bluebuild` | `8ab81e5` | cancelled | superseded by the next push — `concurrency.cancel-in-progress` cancels the older run for the same ref, which is the intended behaviour and costs nothing |
...
**ARM apps and the libhoudini time bomb.** Android apps that ship ARM-only native libraries need a translation layer, and the long-standing Intel-friendly choice — libhoudini — is dead: its builds carry an expiry that fired on **2026-01-01**, after which translated apps hang on their splash screen at ~100 % CPU with nothing in the logs (`waydroid-helper#78`, the canonical thread, tested every houdini variant on vanilla and GAPPS Android 13; `casualsnek/waydroid_script#257` confirms the date). `libndk` is the only live layer, with caveats (some translated apps mis-handle taps, some games crash), and no reports of it on Android 16 exist yet — so `z13-waydroid-setup` recommends libndk, forbids libhoudini with the reason, and points at `--stock` (Android 13 GAPPS) as the known-good fallback if ARM-only apps misbehave.

**Why the ISO builds BCI directly.** `bluebuild generate-iso` pins build-container-installer **v1.4.0** (`blue-build/cli` `utils/src/constants.rs`, unchanged on `main`), and BCI only gained Fedora 44 support in **v1.5.0** ("Upgrade to fedora 44", PR #196, merged 2026-05-03) — which is why both ISO attempts died after pulling the image. The ISO job therefore runs `ghcr.io/jasonn3/build-container-installer:v1.5.0` directly - and after two silent mid-build failures it now mirrors **the action's** invocation rather than the CLI's: the make target (`build/<iso>-Checksum`) comes first, the workspace mounts at `/github/workspace`, `ISO_NAME` is an absolute path inside it, `/cache` is pre-created 0777, the full env set is passed (including `IMAGE_SIGNED`/`ARCH`/`REPOS`), and it runs under plain `docker`. The CLI's env-only form omits the make target entirely, which is the one difference nobody could see from outside (`generate_iso.rs`: same env, same output mount, `ISO_NAME=build/<name>`), and v1.5.0 additionally carries the >128-layer fix. Revert to `bluebuild generate-iso` once the CLI moves that pin.
